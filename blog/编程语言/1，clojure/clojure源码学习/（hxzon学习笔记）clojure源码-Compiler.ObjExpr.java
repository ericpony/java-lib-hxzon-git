//（hxzon学习笔记）clojure源码-Compiler.ObjExpr.java
        ...
        */
        ...
        for (ISeq s = RT.keys(closes); s != null; s = s.next()) {
            ...
            if (isDeftype()) {
                ...
                if (lb.getPrimitiveType() != null)
                    cv.visitField(0 + (isVolatile(lb) ? ACC_VOLATILE : 0), lb.name, Type.getType(lb.getPrimitiveType()).getDescriptor(), null, null);
                else
...
//          getCompiledClass();
    ...
    private void emitKeywordCallsites(GeneratorAdapter clinitgen) {
        for (int i = 0; i < keywordCallsites.count(); i++) {
            ...
            clinitgen.invokeConstructor(KEYWORD_LOOKUPSITE_TYPE, Method.getMethod("void <init>(clojure.lang.Keyword)"));
            clinitgen.dup();
    ...
    void emitValue(Object value, GeneratorAdapter gen) {
        ...
        if (value == null)
            ...
            gen.invokeStatic(Type.getType(Symbol.class), Method.getMethod("clojure.lang.Symbol intern(String,String)"));
...
//                              }
            ...
            if (vs == null)
                ...
                gen.invokeStatic(Type.getType(PersistentHashSet.class), Method.getMethod("clojure.lang.PersistentHashSet create(Object[])"));
            }
...
//      getCompiledClass();
    ...
    public Class getJavaClass() {
        return (compiledClass != null) ? compiledClass : (tag != null) ? HostExpr.tagToClass(tag) : IFn.class;
    }
