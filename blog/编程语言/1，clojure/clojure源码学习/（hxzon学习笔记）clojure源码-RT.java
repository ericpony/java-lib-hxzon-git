...
//static final Namespace USER_NS = Namespace.findOrCreate(Symbol.intern("user"));
    final static public Var OUT = Var.intern(CLOJURE_NS, Symbol.intern("*out*"), new OutputStreamWriter(System.out)).setDynamic();
    final static public Var IN = Var.intern(CLOJURE_NS, Symbol.intern("*in*"), new LineNumberingPushbackReader(new InputStreamReader(System.in))).setDynamic();
    final static public Var ERR = Var.intern(CLOJURE_NS, Symbol.intern("*err*"), new PrintWriter(new OutputStreamWriter(System.err), true)).setDynamic();
...
//boolean
    static final public Var UNCHECKED_MATH = Var.intern(Namespace.findOrCreate(//
...
//symbol
    final public static Var CURRENT_NS = Var.intern(CLOJURE_NS, Symbol.intern("*ns*"), CLOJURE_NS).setDynamic();
...
// duck typing stderr plays nice with e.g. swank 
    ...
    static public void load(String scriptbase, boolean failIfNotFound) throws IOException, ClassNotFoundException {
        ...
        if ((classURL != null && (cljURL == null || lastModified(classURL, classfile) > lastModified(cljURL, cljfile))) || classURL == null) {
            try {
                Var.pushThreadBindings(RT.mapUniqueKeys(CURRENT_NS, CURRENT_NS.deref(), WARN_ON_REFLECTION, WARN_ON_REFLECTION.deref(), RT.UNCHECKED_MATH, RT.UNCHECKED_MATH.deref()));
...
////////////// Collections support /////////////////////////////////
    ...
    static ISeq seqFrom(Object coll) {
        if (coll instanceof Seqable) {
            ...
            throw new ExceptionInfo("Don't know how to create ISeq from: " + c.getName(), map(Keyword.intern("instance"), coll));
        }
    }
