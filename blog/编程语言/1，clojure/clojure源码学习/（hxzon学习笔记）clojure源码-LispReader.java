//（hxzon学习笔记）clojure源码-LispReader.java
...
//static Symbol SYNTAX_QUOTE = Symbol.intern(null, "syntax-quote");//语法引述
    static Symbol UNQUOTE = Symbol.intern("clojure.core", "unquote");//反引述
    static Symbol UNQUOTE_SPLICING = Symbol.intern("clojure.core", "unquote-splicing");//编接反引述
...
//sorted-map num->gensymbol
    ...
    static {//读者宏
        macros['"'] = new StringReader();//字符串字面量
        macros[';'] = new CommentReader();//单行注释
        macros['\''] = new WrappingReader(QUOTE);//引述
        macros['@'] = new WrappingReader(DEREF);//new DerefReader();
        macros['^'] = new MetaReader();//元数据字面量
        macros['`'] = new SyntaxQuoteReader();//语法引述
        macros['~'] = new UnquoteReader();//反引述
        macros['('] = new ListReader();//列表字面量
        macros[')'] = new UnmatchedDelimiterReader();
        macros['['] = new VectorReader();//向量字面量
        macros[']'] = new UnmatchedDelimiterReader();
        macros['{'] = new MapReader();//映射字面量
        macros['}'] = new UnmatchedDelimiterReader();
//        macros['|'] = new ArgVectorReader();
        macros['\\'] = new CharacterReader();
        macros['%'] = new ArgReader();//函数字面量的参数
        macros['#'] = new DispatchReader();

        //井号宏
        dispatchMacros['^'] = new MetaReader();//元数据
        dispatchMacros['\''] = new VarReader();//获取变量本身，#'
        dispatchMacros['"'] = new RegexReader();//正则表达式字面量
        dispatchMacros['('] = new FnReader();//函数字面量
        dispatchMacros['{'] = new SetReader();//集合字面量
        dispatchMacros['='] = new EvalReader();//求值
        dispatchMacros['!'] = new CommentReader();//注释
        dispatchMacros['<'] = new UnreadableReader();//不可达？
        dispatchMacros['_'] = new DiscardReader();//忽略整个形式
    }
                        ...
                        GENSYM_ENV.set(//
                                gmap.assoc(sym, //
                                        gs = Symbol.intern(null, //
                                                sym.name.substring(0, sym.name.length() - 1) + "__" + RT.nextID() + "__auto__")));
                    }
                    sym = gs;
    ...
    public static class CharacterReader extends AFn {
        public Object invoke(Object reader, Object backslash) {
            ...
            if (token.length() == 1) {
                ...
                return c;
            } else if (token.startsWith("o")) {//八进制字符字面量
...
//自定义的读者宏
    public static class CtorReader extends AFn {
        public Object invoke(Object reader, Object firstChar) {
            ...
            if (!(name instanceof Symbol)) {//读者宏的宏字符必须是一个符号
